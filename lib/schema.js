...
function schema_jsonSchema(name) {
  ...
  if (this.__buildingSchema) {
    this.__jsonSchemaId = this.__jsonSchemaId || '#schema-' + ++schema_jsonSchema.__jsonSchemaIdCounter;
  ...
  if (!this.__jsonSchema) {
    ...
    if (this.__jsonSchemaId) {
      this.__jsonSchema = Object.assign({ id: this.__jsonSchemaId }, this.__jsonSchema);
    }
  }
...
function __build(name, schema) {
  ...
  for (path of paths) {
    ...
    sch = schema.paths[path].jsonSchema(jss.prop, schema);
    ...
    jss.cont.additionalProperties = false;
  }

  if (result.required.length === 0) delete result.required;

  var needToAddVirtuals =
    schema.options && schema.options.toJSON && (schema.options.toJSON.virtuals || schema.options.toJSON.getters);
